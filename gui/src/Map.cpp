...
{
    return {
        (static_cast<float>(x) - static_cast<float>(state.getMapWidth()) / 2.0f) * _squareSize,
        height,
        (static_cast<float>(y) - static_cast<float>(state.getMapHeight()) / 2.0f) * _squareSize,
    };
}
...
void Map::draw(const GameState &state) const
{
    ...
    for (std::size_t row = 0; row < state.getMapHeight(); ++row) {
        for (std::size_t col = 0; col < state.getMapWidth(); ++col) {
    ...
    for (const Tile &tile : state.getTiles()) {
        int resourceCount = 0;

        for (int quantity : tile.getResources())
    ...
    for (const auto &[id, player] : state.getPlayers()) {
        (void)id;
