...
PLAYER_SOURCE = ROOT / "player.glb"

# Blender exports Z-up scenes to glTF/Raylib Y-up. This offset keeps the retargeted
# idle/walk pose visually grounded on the GUI tile after export.
...
# The last value is a strength: full humanoid motion is too aggressive for this
# stylized penguin, so each target bone only receives the useful part of the source
# local rotation.
BONE_MAP = [
...
def retarget_animation(target_armature, fbx_name, action_name):
    source_armature, source_objects, source_actions = import_source_animation(ROOT / fbx_name)
...
def export_combined_player(target_armature):
...
def main():
    ...
    clear_scene()
    target_armature = import_base_player()
    ...
    for fbx_name, action_name in ANIMATIONS:
        retarget_animation(target_armature, fbx_name, action_name)

    export_combined_player(target_armature)
